...
internal static class Program
{
    ...
    private static GlyphCandidate[] BuildAtlas(FontFamily family)
    {
        ...
        for (int codePoint = 32; codePoint <= 126; codePoint++)
        {
            ...
            using (Graphics graphics = Graphics.FromImage(raster))
            {
                ...
                graphics.DrawString(character.ToString(), font, Brushes.White,
                    new RectangleF(0, 0, rasterWidth, rasterHeight), StringFormat.GenericTypographic);
            }
            double[] mask = ResizeToPatch(raster, PatchWidth, PatchHeight);
            double[] transformed = Dct2(mask, PatchWidth, PatchHeight);
            result.Add(new GlyphCandidate(character, transformed, Dot(transformed, transformed)));
        }
    ...
    private static void Render(string sourcePath, string outputDirectory, FontFamily family, GlyphCandidate[] atlas)
    {
        ...
        for (int cellX = 0; cellX < OutputColumns; cellX++)
        {
            ...
            double[][] transformed = patch.Select(channel => Dct2(channel, PatchWidth, PatchHeight)).ToArray();
            cells[cellY, cellX] = Match(transformed, atlas);
        }
    ...
    private static MatchedCell Match(double[][] patch, IReadOnlyList<GlyphCandidate> atlas)
    {
        ...
        foreach (GlyphCandidate glyph in atlas)
        {
            ...
            {
                double patchEnergy = Dot(patch[channel], patch[channel]);
                ...
                score += patchEnergy - 2 * foreground * correlation + foreground * foreground * glyph.Energy;
            }
        ...
        return new MatchedCell(best.Character,
    ...
    {
        double sum = 0;
        for (int index = 0; index < left.Count; index++) sum += left[index] * right[index];
        return sum;
    }
    ...
    private static void WriteText(string path, MatchedCell[,] cells)
    {
        ...
        {
            var value = new char[OutputColumns];
            for (int column = 0; column < OutputColumns; column++) value[column] = cells[row, column].Character;
            lines[row] = new string(value).TrimEnd();
        }
    ...
    private static void WriteAnsi(string path, MatchedCell[,] cells)
    {
        ...
        for (int row = 0; row < OutputRows; row++)
        {
            for (int column = 0; column < OutputColumns; column++)
            {
                ...
                output.Append($"\x1b[38;2;{cell.Color.R};{cell.Color.G};{cell.Color.B}m{cell.Character}");
            }
    ...
    private static void WritePreview(string path, MatchedCell[,] cells, FontFamily family)
    {
        ...
        for (int column = 0; column < OutputColumns; column++)
        {
            ...
            graphics.DrawString(cell.Character.ToString(), font, brush,
    ...
    private sealed record GlyphCandidate(char Character, double[] Dct, double Energy);
    private sealed record MatchedCell(char Character, Color Color);
}
